...
var BULLET_SOUND = loadSound("bulletsound.mp3");
var EXPLOSION_SOUND = loadSound("explosion.mp3");
...
function onSetup() {
  ...
  setInterval(function(){
    var counter = 1;
    if(!resetFlag){
      while(length(enemies) < 12){
        buildEnemy(screenWidth/6 + counter*115, 0);
        counter++;
      }
    }
  }, 6000);
  ...
  setInterval(function(){
    var shootingenemy = randomInteger(0,length(enemies)+1);
    if(shootingenemy != indexOf(destroyedEnemies, shootingenemy) && length(enemies)>0){
      buildEnemyBullets(enemies[shootingenemy].pos.x,enemies[shootingenemy].pos.y);
    }
  }, 8000);
...
function doPlayerColl(){
  for(i=0;i<length(enemyBullets);i++){
    for(j=0;j<length(enemies);j++){
        if(collides(player,enemyBullets[i]) || collides(player, enemies[j])){
